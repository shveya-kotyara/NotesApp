...
function download(url, dest, cb) {
  var reportError = function reportError(err) {
    ...
    if (err.code === 'ETIMEDOUT') {
      if (err.connect === true) {
        ...
        timeoutMessge = 'Timed out whilst downloading the prebuilt binary'; // occurs any time the server is too slow to send back a part of the response
      }
    }
  ...
  var successful = function successful(response) {
  ...
  try {
    request(url, downloadOptions(), function (err, response, buffer) {
      ...
      var progress = log.newItem('', length); // The `progress` is true by default. However if it has not
  ...
  download(sass.getBinaryUrl(), binaryPath, function (err) {
    ...
    if (cachePath) {
      ...
      try {
        ...
        fs.createReadStream(binaryPath).pipe(fs.createWriteStream(cachedBinary)).on('error', function (err) {
