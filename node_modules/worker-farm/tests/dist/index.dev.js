...
tape('on child', function (t) {
  ...
  var child = workerFarm({
    onChild: function onChild(subprocess) {
  ...
  setTimeout(function () {
    ...
    while (i--) {
      child(defer, function () {
        if (++cbc == count) {
          var time = Date.now() - start; // upper-limit not tied to `count` at all
...
tape('single concurrent call', function (t) {
  ...
  setTimeout(function () {
    ...
    while (i--) {
      child(defer, function () {
        if (++cbc == count) {
          var time = Date.now() - start; // upper-limit tied closely to `count`, 1.3 is generous but accounts for all the timers
    ...
    while (i--) {
      child(defer, function () {
        if (++cbc == count) {
          ...
          var min = defer * 1.5; // (defer * (count / callsPerWorker + 2)) - if precise it'd be count/callsPerWorker
          // but accounting for IPC and other overhead, we need to give it a bit of extra time,
          // hence the +2

          var max = defer * (count / callsPerWorker + 2);
