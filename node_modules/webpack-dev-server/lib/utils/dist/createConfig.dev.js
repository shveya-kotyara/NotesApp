...
function createConfig(config, argv, _ref) {
  var port = _ref.port;
  var firstWpOpt = Array.isArray(config) ? config[0] : config;
  var options = firstWpOpt.devServer || {}; // This updates both config and firstWpOpt
  ...
  if (!options.publicPath) {
    ...
    if (!isAbsoluteUrl(String(options.publicPath)) && options.publicPath[0] !== '/') {
      options.publicPath = "/".concat(options.publicPath);
    }
  }
  ...
  if (argv.contentBase) {
    ...
    if (Array.isArray(options.contentBase)) {
      options.contentBase = options.contentBase.map(function (p) {
  ...
  if (_typeof(options.stats) === 'object' && typeof options.stats.colors === 'undefined' && argv.color) {
