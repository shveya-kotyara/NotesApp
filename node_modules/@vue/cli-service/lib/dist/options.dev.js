...
var schema = createSchema(function (joi) {
  return joi.object({
    ...
    pages: joi.object().pattern(/\w+/, joi.alternatives()["try"]([joi.string().required(), joi.array().items(joi.string().required()), joi.object().keys({
      entry: joi.alternatives()["try"]([joi.string().required(), joi.array().items(joi.string().required())]).required()
    }).unknown(true)])),
...
exports.validate = function (options, cb) {
