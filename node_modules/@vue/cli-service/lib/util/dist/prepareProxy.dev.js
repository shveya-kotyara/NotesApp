...
module.exports = function prepareProxy(proxy, appPublicFolder) {
  ...
  if (Array.isArray(proxy) || _typeof(proxy) !== 'object' && typeof proxy !== 'string') {
  ...
  function mayProxy(pathname) {
    ...
    var isPublicFileRequest = fs.existsSync(maybePublicPath) && fs.statSync(maybePublicPath).isFile();
    var isWdsEndpointRequest = pathname.startsWith('/sockjs-node'); // used by webpackHotDevClient

    return !(isPublicFileRequest || isWdsEndpointRequest);
  }

  function createProxyEntry(target, usersOnProxyReq, _context) {
    // #2478
    ...
    return {
      ...
      context: function context(pathname, req) {
        ...
        if (_context) {
          ...
          return pathname.match(_context);
        } else {
  ...
  return Object.keys(proxy).map(function (context) {
    var config = proxy[context];

    if (!config.hasOwnProperty('target')) {
      console.log(chalk.red('When `proxy` in package.json is an object, each `context` object must have a ' + '`target` property specified as a url string'));
      process.exit(1);
    }

    var entry = createProxyEntry(config.target, config.onProxyReq, context);
