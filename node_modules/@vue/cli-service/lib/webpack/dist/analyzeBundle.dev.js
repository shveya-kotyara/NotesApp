...
exports.analyzeBundle = function analyzeBundle(bundleStats, assetSources) {
  ...
  var jsAssets = [];
  var otherAssets = []; // Separate JS assets

  bundleStats.assets.forEach(function (asset) {
  ...
  for (var _i = 0, _jsAssets = jsAssets; _i < _jsAssets.length; _i++) {
    var asset = _jsAssets[_i];
    var source = assetSources.get(asset.name);
    ...
    if (!bundleInfo) {
      warn("\nCouldn't parse bundle asset \"".concat(asset.fullPath, "\".\n") + 'Analyzer will use module sizes from stats file.\n');
      parsedModules = null;
      bundlesSources = null;
      break;
    }
  ...
  bundleStats.modules.forEach(function (module) {
    var parsedSrc = parsedModules && parsedModules[module.id];
    module.size = {
      stats: module.size
    };
  ...
  jsAssets.forEach(function (asset) {
    var src = bundlesSources && bundlesSources[asset.name];
    asset.size = {
      stats: asset.size
    };
  ...
  otherAssets.forEach(function (asset) {
    var src = assetSources.get(asset.name);
    asset.size = {
      stats: asset.size,
      parsed: asset.size
    };
...
function parseBundle(bundleContent) {
  var ast = acorn.parse(bundleContent, {
    sourceType: 'script',
  ...
  walk.recursive(ast, walkState, {
    CallExpression: function CallExpression(node, state, c) {
      ...
      var args = node.arguments; // Additional bundle without webpack loader.
  ...
  return {
    src: bundleContent,
    modules: mapValues(walkState.locations, function (loc) {
      return bundleContent.slice(loc.start, loc.end);
    })
  };
}
...
function isArgumentContainsModulesList(arg) {
  if (arg.type === 'ObjectExpression') {
    return arg.properties.map(function (prop) {
      return prop.value;
  ...
  if (arg.type === 'ArrayExpression') {
    ...
    return arg.elements.every(function (elem) {
      return (// Some of array items may be skipped because there is no module with such id
...
function isArgumentArrayConcatContainingChunks(arg) {
  if (arg.type === 'CallExpression' && arg.callee.type === 'MemberExpression' && // Make sure the object called is `Array(<some number>)`
  arg.callee.object.type === 'CallExpression' && arg.callee.object.callee.type === 'Identifier' && arg.callee.object.callee.name === 'Array' && arg.callee.object.arguments.length === 1 && isNumericId(arg.callee.object.arguments[0]) && // Make sure the property X called for `Array(<some number>).X` is `concat`
  arg.callee.property.type === 'Identifier' && arg.callee.property.name === 'concat' && // Make sure exactly one array is passed in to `concat`
...
function getModulesLocationFromFunctionArgument(arg) {
  if (arg.type === 'ObjectExpression') {
    ...
    return transform(modulesNodes, function (result, moduleNode) {
  ...
  if (arg.type === 'ArrayExpression') {
    ...
    return transform(_modulesNodes, function (result, moduleNode, i) {
...
function getModulesLocationFromArrayConcat(arg) {
  ...
  var minId = arg.callee.object.arguments[0].value; // The modules reside in the `concat()` function call arguments

  var modulesNodes = arg.arguments[0].elements;
  return transform(modulesNodes, function (result, moduleNode, i) {
    if (!moduleNode) return;
