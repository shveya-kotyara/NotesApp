...
var rc = function rc(ctx, path, options) {
  /**
  ...
  return config('postcss', options).search(path).then(function (result) {
    if (!result) {
...
rc.sync = function (ctx, path, options) {
  /**
   * @type {Object} The full Config Context
   */
  ctx = createContext(ctx);
  ...
  path = path ? resolve(path) : process.cwd();
  var result = config('postcss', options).searchSync(path);
