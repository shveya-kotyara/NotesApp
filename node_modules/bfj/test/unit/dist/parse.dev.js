...
suite('parse:', function () {
  ...
  suite('require:', function () {
    ...
    test('parse does not throw if reviver is an object', function () {
      assert.doesNotThrow(function () {
        parse({}, {
          reviver: {}
        })["catch"](function () {});
    ...
    test('parse returns a different type of promise if the option is set', function () {
    ...
    test('parse rejects immediately if reviver is an object', function () {
      return parse({}, {
        reviver: {}
      }).then(function () {
        return assert(false);
    ...
    test('parse does not reject immediately if reviver is a function', function () {
      parse({}, {
        reviver: function reviver() {}
      })["catch"](function (error) {
        return assert(false);
    ...
    suite('parse with ndjson:', function () {
      var stream;
      setup(function () {
        stream = {};
        parse(stream, {
      ...
      test('EventEmitter.on was called twelve times', function () {
        assert.strictEqual(log.counts.on, 12);
        assert.strictEqual(log.these.on[11], results.walk[0]);
      });
      test('EventEmitter.on was called correctly first eleven times', function () {
        assert.strictEqual(log.args.on[0][0], 'arr');
        assert.strictEqual(log.args.on[1][0], 'obj');
        assert.strictEqual(log.args.on[2][0], 'pro');
        assert.strictEqual(log.args.on[3][0], 'str');
      ...
      test('EventEmitter.on was called correctly twelfth time', function () {
      ...
      test('emitter.pause was not called', function () {
        assert.strictEqual(log.counts.pause, 0);
      });
      test('emitter.removeAllListeners was not called', function () {
        assert.strictEqual(log.counts.removeAllListeners, 0);
      });
      suite('array, endArray, endLine:', function () {
        setup(function (done) {
          log.args.on[0][1]();
          log.args.on[6][1]();
          log.args.on[11][1]();
        ...
        test('emitter.removeAllListeners was called once', function () {
          assert.strictEqual(log.counts.removeAllListeners, 1);
        });
        test('emitter.removeAllListeners was called correctly', function () {
        ...
        suite('parse with ndjson and fresh stream:', function () {
          setup(function () {
            parse({}, {
              ndjson: true
            }).then(spooks.fn({
