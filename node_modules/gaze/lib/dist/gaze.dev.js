...
var helper = require('./helper'); // shim setImmediate for node v0.8
...
var delay = 10; // `Gaze` EventEmitter object to return in the callback

function Gaze(patterns, opts, done) {
  var self = this;
  EE.call(self); // If second arg is the callback
  ...
  this.options = opts; // Default done callback

  done = done || function () {}; // Remember our watched dir:files


  this._watched = Object.create(null); // Store watchers

  this._watchers = Object.create(null); // Store watchFile listeners

  this._pollers = Object.create(null); // Store patterns

  this._patterns = []; // Cached events for debouncing

  this._cached = Object.create(null); // Set maxListeners
...
util.inherits(Gaze, EE); // Main entry point. Start watching and call done when setup

module.exports = function gaze(patterns, opts, done) {
  return new Gaze(patterns, opts, done);
};

module.exports.Gaze = Gaze; // Override the emit function to emit `all` events
...
Gaze.prototype.emit = function () {
  ...
  var e = args[0];
  var filepath = args[1];
  var timeoutId; // If not added/deleted/changed/renamed then just emit the event
  ...
  if (cache.indexOf(e) === -1) {
    ...
    }, this.options.debounceDelay); // Emit the event and `all` event
...
Gaze.prototype._addToWatched = function (files) {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    dirname = helper.markDir(dirname); // If a new dir is added
  ...
  for (var k = 0; k < dirs.length; k++) {
    dirname = dirs[k]; // add folders into the mix
...
Gaze.prototype._initWatched = function (done) {
  var self = this;
  var cwd = this.options.cwd || process.cwd();
  var curWatched = Object.keys(self._watched); // if no matching files
  ...
  helper.forEachSeries(curWatched, function (dir, next) {
    dir = dir || '';
    var files = self._watched[dir]; // Triggered when a watched dir has an event
    ...
    self._watchDir(dir, function (event, dirpath) {
      ...
      fs.readdir(dirpath, function (err, current) {
        ...
        } catch (err) {} // race condition-- sometimes the file no longer exists
        // Get watched files for this dir


        var previous = self.relative(relDir); // If file was deleted
        ...
        current.filter(function (file) {
          ...
          var relFile = path.join(relDir, file); // Add to watch then emit event
