...
var minimatch = require('minimatch'); // The module.


var globule = exports; // Process specified wildcard glob patterns or filenames against a
...
globule.match = function (patterns, filepaths, options) {
  ...
  filepaths = _.isArray(filepaths) ? _.flattenDeep(filepaths) : [filepaths]; // Return empty set if there are no patterns or filepaths.
...
globule.find = function () {
  var args = _.toArray(arguments); // If the last argument is an options object, remove it from args.


  var options = _.isPlainObject(args[args.length - 1]) ? args.pop() : {}; // If options.src was specified, use it. Otherwise, use all non-options
  ...
  var srcBase = options.srcBase || options.cwd; // Create glob-specific options object.
...
globule.mapping = function (filepaths, options) {
  ...
  filepaths.forEach(function (src) {
    ...
    var dest = options.rename(src, options); // Prepend srcBase to all src paths.
    ...
    src = normalizePath(src); // Map correct src path to dest path.
...
globule.findMapping = function () {
  ...
  var options = _.isPlainObject(args[args.length - 1]) ? args.pop() : {}; // Generate mapping from found filepaths.
