...
module.exports.dependency = function (chunks, options, compilation) {
  ...
  if (chunkGroups) {
    ...
    edges = chunkGroups.reduce(function (result, chunkGroup) {
      return result.concat(Array.from(chunkGroup.parentsIterable, function (parentGroup) {
        return [parentGroup, chunkGroup];
      }));
    }, []);
    var sortedGroups = toposort.array(chunkGroups, edges); // flatten chunkGroup into chunks
    ...
    var sortedChunks = sortedGroups.reduce(function (result, chunkGroup) {
      ...
      var exists = !!chunk; // make sure we have a unique list
    ...
    chunks.forEach(function (chunk) {
      if (chunk.parents) {
        ...
        chunk.parents.forEach(function (parentId) {
          ...
          var parentChunk = _.isObject(parentId) ? parentId : nodeMap[parentId]; // If the parent chunk does not exist (e.g. because of an excluded chunk)
...
module.exports.manual = function (chunks, options) {
  var specifyChunks = options.chunks;
  var chunksResult = [];
  var filterResult = [];
